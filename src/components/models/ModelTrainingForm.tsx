...
const ModelTrainingForm = () => {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      console.log("returned from signed url ", data);
      if (data.error) {
      ...
      if (!response.ok || results?.error) {
        console.log("Error from await", results?.error);
        throw new Error(results?.error || "Failed to train the model");
      }

      toast.success(
        "Training started successfully. You'll receive a notification once completed.",
        { id: toastId }
      );
    } catch (error) {
      const errorMessage =
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        <fieldset className="grid max-w-5xl bg-background p-8 rouded-lg gap-6 border">
          <FormField
              ...
              <FormItem>
                ...
                <div className="mb-4 rounded-lg shadow-sm pb-4 text-card-foreground">
                  <ul className="space-y-2 text-sm text-muted-foreground">
                    ...
                    <li>
                      • No accessories on face/head ideally • No other people in
                      images
                    </li>
                    <li>
